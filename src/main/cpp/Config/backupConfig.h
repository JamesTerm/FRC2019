...
private:
	...
	void AddPIDProfile(string Name, double P, double I, double D, double MaxChange = 0.1, double Bias = -1, double InnerMin = 0, double InnerMax = 0, double Min = -1, double Max = 1, double Thres = 0.01, int index = -1)
	{
		int IndexAt = m_activeCollection->CreateAndAddProfile(Name, P, I, D, MaxChange, (Bias <= 0 ? P * 100 : Bias), InnerMin, InnerMax, Min, Max, Thres, index);
